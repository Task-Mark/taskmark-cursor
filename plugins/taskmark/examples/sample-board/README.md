...
Optional (opt-in): add a stub `package.json` with `"start": "taskmark serve"` so `npm start` works — see `examples/board-ui-stub/package.json`. Boards stay markdown-first by default.
