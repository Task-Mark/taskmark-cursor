...
3. Set `size: null` (epics have **no** t-shirt size). Set `points: 0` until children exist — points always **sum of child story points plus epic-direct task/bug points**. `estimate_minutes: 0`, `actual_minutes: 0`, `session_cap_minutes: 480`, `points_source: rolled_up`.
