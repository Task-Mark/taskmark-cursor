...
- **Done items must have a work log:** never leave `status: done` with an empty Work log table. Billable total must be **> 2** minutes for delivered work **unless** the row is a **shared-batch** allocation (see [effort-time](effort-time.md)) whose batch total is > 2 — leaf slices may then be ≤ 2. If historical delivery had no log, backfill one closed session from known work duration (or a conservative ≥5 minute stub), never a ≤2 minute stub. Summary may note `auto-backfilled: missing work log`.
